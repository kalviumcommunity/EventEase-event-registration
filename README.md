...
Implementing deployment verification and rollback strategies significantly improves key DevOps metrics:

| Metric                          | Definition                          | Target   | Improvement Strategy                                                                                                                                |
| ------------------------------- | ----------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MTTD (Mean Time to Detect)**  | Time to identify a failure          | < 5 min  | Automated **Health Checks** and **Smoke Tests** run immediately after deployment, detecting issues in seconds rather than waiting for user reports. |
| **MTTR (Mean Time to Recover)** | Time to recover from failure        | < 30 min | Automated **Rollback** scripts restore the previous stable version instantly upon verification failure, minimizing downtime.                        |
| **Change Failure Rate (CFR)**   | % of deployments requiring rollback | < 15%    | **Verification Steps** in the pipeline prevent broken code from staying live, ensuring only healthy builds stick.                                   |

### Verification & Rollback Strategy

1.  **Health Check**: A lightweight `/api/health` endpoint reports service status `200 OK`.
2.  **Smoke Tests**: Critical user flows (e.g., Homepage Load) are validated using Jest immediately post-deploy.
3.  **Rollback Logic**: If manual steps or verification fail, the pipeline automatically triggers a rollback command (e.g., via AWS ECS or Azure CLI) to revert to the last successful deployment.
